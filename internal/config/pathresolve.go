...
func ResolvePath(raw string) (string, bool) {
	...
	segmentsFound := 0
	lastMatchIdx := len(norm)

	currentEnd := len(norm)
	for currentEnd > 0 {
		idx := strings.LastIndexByte(norm[:currentEnd], '/')

		var segment string
		if idx == -1 {
			segment = norm[:currentEnd]
		} else {
			segment = norm[idx+1 : currentEnd]
		}
	...
	if segmentsFound >= 2 {
		return "/" + strings.TrimLeft(norm[lastMatchIdx:], "/"), true
	}
	return raw, true
}
