...
func TestCLI_EdgeCases(t *testing.T) {
	...
	// Pre-set GITHUB_TOKEN for sync tests to avoid "GITHUB_TOKEN env var required" error
	...
	// 9b. Sync: github error with repo (results in API error due to dummy token)
	_, err = executeCmd("sync", "--to=github", "--github-repo=owner/repo")
	assert.Error(t, err)
	// Expect some kind of API or network error since we have a token but no mock server here
	assert.Contains(t, err.Error(), "sync failed for github")
