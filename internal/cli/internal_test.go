...
// TestPrintSecrets_EmptyJSON_HasStderrHintAndEmptyArray verifies that even
// for JSON output the user receives a "no secrets" hint on stderr while
// stdout still contains a parseable empty array — preserving the contract
// for scripts that grep on it.
func TestPrintSecrets_EmptyJSON_HasStderrHintAndEmptyArray(t *testing.T) {
	cmd := &cobra.Command{}
	var out, errBuf bytes.Buffer
	cmd.SetOut(&out)
	cmd.SetErr(&errBuf)

	err := printSecrets(cmd, nil, "json", false)
	require.NoError(t, err)
	assert.Contains(t, errBuf.String(), "No secrets found")
	assert.Equal(t, "[]\n", out.String(), "stdout must remain a valid empty JSON array")
}

// TestPrintSecrets_EmptyTable_NoBody verifies that the table format still
// short-circuits to the stderr hint without emitting an empty header.
func TestPrintSecrets_EmptyTable_NoBody(t *testing.T) {
	cmd := &cobra.Command{}
	var out, errBuf bytes.Buffer
	cmd.SetOut(&out)
	cmd.SetErr(&errBuf)

	err := printSecrets(cmd, nil, "table", false)
	require.NoError(t, err)
	assert.Contains(t, errBuf.String(), "No secrets found")
	assert.Empty(t, out.String())
}

// TestPrintEnvPairs_EmptyJSON_HasStderrHintAndEmptyObject ensures `skret env
// --format json` still emits `{}` on stdout when there are no secrets, with
// a human hint on stderr.
func TestPrintEnvPairs_EmptyJSON_HasStderrHintAndEmptyObject(t *testing.T) {
	cmd := &cobra.Command{}
	var out, errBuf bytes.Buffer
	cmd.SetOut(&out)
	cmd.SetErr(&errBuf)

	err := printEnvPairs(cmd, nil, "json")
	require.NoError(t, err)
	assert.Contains(t, errBuf.String(), "No secrets found")
	assert.Equal(t, "{}\n", out.String())
}

// TestPrintEnvPairs_EmptyYAML_HasStderrHintAndEmptyDoc ensures yaml emits
// an empty doc and the stderr hint.
func TestPrintEnvPairs_EmptyYAML_HasStderrHintAndEmptyDoc(t *testing.T) {
	cmd := &cobra.Command{}
	var out, errBuf bytes.Buffer
	cmd.SetOut(&out)
	cmd.SetErr(&errBuf)

	err := printEnvPairs(cmd, nil, "yaml")
	require.NoError(t, err)
	assert.Contains(t, errBuf.String(), "No secrets found")
	// yaml.Marshal of an empty map produces "{}\n".
	assert.Equal(t, "{}\n", out.String())
}

// TestPrintEnvPairs_EmptyDotenv_OnlyStderrHint confirms dotenv still returns
// nil with only a hint on stderr, no body.
func TestPrintEnvPairs_EmptyDotenv_OnlyStderrHint(t *testing.T) {
	cmd := &cobra.Command{}
	var out, errBuf bytes.Buffer
	cmd.SetOut(&out)
	cmd.SetErr(&errBuf)

	err := printEnvPairs(cmd, nil, "dotenv")
	require.NoError(t, err)
	assert.Contains(t, errBuf.String(), "No secrets found")
	assert.Empty(t, out.String())
}
