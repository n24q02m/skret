	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if !dryRun && (onConflict == "skip" || onConflict == "fail") {
				// Optimization: Fetch all existing secrets once to avoid N+1 Get calls.
				// This improves performance and reduces pressure on the provider API.
			...
			for _, s := range secrets {
				...
				if onConflict == "skip" || onConflict == "fail" {
					...
					if listLoaded {
						...
						// Fallback to Get if List failed or wasn't loaded
