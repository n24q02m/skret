...
func NewGitHubSource(owner, repo, token, baseURL string) Source {
	...
	// Use a custom http.Client with a timeout to prevent connection hangs.
	client := &http.Client{Timeout: 30 * time.Second}
	return githubSource{owner: owner, repo: repo, token: token, baseURL: baseURL, client: client}
}
