...
// TestSaveSyncState_PathTraversal verifies that even with a malicious id,
// the file is written inside the sync-state directory and the attacker can't
// land a file at /tmp/evil or sibling directories.
func TestSaveSyncState_PathTraversal(t *testing.T) {
	...
	// Confirm nothing was written above sync-state.
	...
	// Confirm one file was created inside sync-state.
