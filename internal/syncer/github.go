...
func (g *GitHubSyncer) Sync(ctx context.Context, secrets []*provider.Secret) error {
	...
	// ⚡ Bolt: Deduplicate secrets by key (last value wins)
	uniqueSecrets := make(map[string]*provider.Secret, len(secrets))
	...
	// ⚡ Bolt: Extract invariant base64 decode and validation outside the concurrent loop
