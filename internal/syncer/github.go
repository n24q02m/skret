...
func (g *GitHubSyncer) Sync(ctx context.Context, secrets []*provider.Secret) error {
	...
	// Deduplicate secrets by key to prevent redundant network calls
	deduped := make([]*provider.Secret, 0, len(secrets))
	...
	// Reverse back to original relative order (last one wins)
	...
	// Base64 decode once outside the loop instead of for every secret
	...
	errCh := make(chan error, len(deduped))
...
func (g *GitHubSyncer) putSecret(ctx context.Context, name, value string, recipientKey *[32]byte, keyID string) error {
	encValue, err := sealSecretBytes(value, recipientKey)
	if err != nil {
...
// sealSecretBytes encrypts a secret using NaCl sealed box (curve25519 + xsalsa20-poly1305).
// This matches GitHub's required encryption format for Actions secrets.
func sealSecretBytes(secret string, recipientKey *[32]byte) (string, error) {
