...
func (g *GitHubSyncer) Sync(ctx context.Context, secrets []*provider.Secret) error {
	...
	for _, s := range secrets {
		...
		go func(s *provider.Secret) {
			...
			if err := g.putSecret(ctx, s.Key, s.Value, keyID, &pubKey); err != nil {
...
func (g *GitHubSyncer) putSecret(ctx context.Context, name, value, keyID string, pubKey *[32]byte) error {
