...
func (g *GitHubSyncer) Sync(ctx context.Context, secrets []*provider.Secret) error {
	...
	// Deduplicate incoming secrets (last value wins) to prevent redundant network calls
	deduped := make(map[string]*provider.Secret)
	...
	// Optimization: Extract invariant base64 decoding outside the concurrent loop
