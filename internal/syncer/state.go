...
// sanitizeIDReplacer is hoisted to a package-level variable to avoid redundant memory
// allocations and initialization overhead on each invocation of sanitizeID.
...
func sanitizeID(id string) string {
	// Performance optimization: fast-path check using ContainsAny skips replacement logic
	// entirely if target characters are absent, reducing allocations to 0 for clean strings.
