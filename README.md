...
=======
- **Multi-provider backend**: AWS SSM Parameter Store, Azure Key Vault, and OCI Vault today; GCP Secret Manager on the roadmap. Switch backends with one config line.
...
| Cloud secret-store backends | AWS SSM, Azure KV, OCI Vault today; GCP SM on roadmap | own store | own store | own store | AWS SM, AWS SSM, GCP SM, Vault, Consul, dotenv | AWS SM/SSM, GCP SM, Azure KV, Vault, SOPS, Bitwarden | Conjur, AWS, keyring (provider plugin) | AWS SSM, AWS Secrets Manager, S3 / S3-KMS (AWS-only) |
