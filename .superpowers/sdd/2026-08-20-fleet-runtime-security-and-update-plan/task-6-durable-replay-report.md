...
- TDD red: `pnpm exec vitest run test/executor-replay-store.test.ts` failed before implementation because `../src/executor-replay-store` did not exist; the follow-up cursor-regression test also failed against prefix-only cursor validation.
- Green: `pnpm exec vitest run test/executor-replay-store.test.ts` — PASS (10 tests, including cursor-resumed and malformed-cursor sweep coverage).
...
- Added bounded transactional `sweep(now, limit, startAfter?)` returning `{ removed, nextAfter }`; it passes an explicit list limit/cursor, validates cursors as exact private SHA-256 keys, and uses no unbounded list/delete operation. A caller can resume after live rows so later expired rows cannot starve.
...
- Bounded expiry sweep, cursor-resumed sweep past live rows that sort before an expired row, and malformed-cursor rejection before storage access.
