...
- `d209189` — `fix: make sync acknowledgements idempotent`
## TDD evidence
...
- Made repeated same-status key acknowledgements and terminal finalization idempotent without rewriting hashes, timestamps, or terminal metadata.
- Added focused coverage for partial success/failure, finalization, idempotence, stale operation/key rejection, unknown/nil/blank inputs, foreign outcomes, interrupted partial operations, and existing batch/round-trip behavior.
