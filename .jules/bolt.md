...
## 2026-08-13 - Avoid split and join for trailing segment extraction
**Learning:** In string parsing routines that only need to inspect or extract the trailing segments of a string (like `ResolvePath`), using `strings.Split(s, delim)` followed by `strings.Join` unconditionally allocates a slice for all segments and constructs a new string, even if most of the string is discarded.
**Action:** Replace `strings.Split` and `strings.Join` with a manual reverse-parsing loop using `strings.LastIndexByte`. Manually scanning backwards and slicing directly from the original string avoids slice allocations completely and significantly improves execution time on the hot path.
