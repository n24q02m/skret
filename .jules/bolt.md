...
## 2026-08-22 - Add inline comments explaining optimizations
**Learning:** Even when an optimization is correct and measurable, omitting comments explaining the *why* and *how* of the optimization (e.g., explaining why a manual reverse-parsing loop is used instead of standard library functions) violates the explicit persona requirement to add comments explaining the optimization. This can lead to PR rejection or revision requests.
**Action:** When implementing any performance optimization, ensure a concise inline comment is added directly above the optimized logic, explaining the rationale (e.g., "Avoid allocations from strings.Split") to satisfy the "✅ Always do: Add comments explaining the optimization" rule.
