...
## 2026-08-21 - Zero-Allocation Path Parsing
**Learning:** Using `strings.Split` followed by `strings.Join` or slicing unconditionally allocates memory and builds new strings. In path parsing routines where we just need to extract trailing segments, we can avoid allocations entirely.
**Action:** Replace `strings.Split(s, delim)` with a manual reverse-parsing loop using `strings.LastIndexByte` to slice directly from the original string, achieving zero allocations and significantly faster execution. When extracting the final path, use `strings.TrimLeft(slice, "/")` instead of manual index adjustments to replicate delimiter-stripping safely.
