...
## 2026-10-31 - Replace strings.Split with strings.Cut loop for path validation
**Learning:** Functions like `validStateManifestPath` use `strings.Split(value, "/")` only to iterate over the resulting segments. Because `strings.Split` returns a slice, it unnecessarily allocates memory on the heap for every function call.
**Action:** Replace `strings.Split` loops with `strings.Cut` loops (e.g. `for { part, rest, found := strings.Cut(remaining, "/") ... }`) in path validation or parsing routines where the resulting slice is not stored, eliminating heap allocations while maintaining readability.
