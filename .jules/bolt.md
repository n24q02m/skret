...
**Learning:** `strings.NewReplacer` allocates memory and builds internal replacement tables upon initialization. When placed inside frequently invoked functions, this redundant setup overhead negatively impacts performance by causing extra memory allocations and CPU usage.
**Action:** Always hoist `strings.NewReplacer` declarations to package-level variables so they are initialized only once, avoiding repeated setup costs during function execution.
