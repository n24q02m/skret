...
## 2026-08-28 - Zero-Allocation Path Extraction
**Learning:** Using strings.Split followed by strings.Join for path parsing unconditionally allocates slices and new strings. In Go, replacing this with a manual reverse-parsing loop (using strings.LastIndexByte) and slicing directly from the original string achieves zero allocations and significantly faster execution on string manipulation.
**Action:** When extracting trailing segments from strings, avoid strings.Split. Instead, scan backward with strings.LastIndexByte, check remaining segment validity if the delimiter is not found (idx == -1), and use strings.TrimLeft(slice, "/") on the resulting original slice.
