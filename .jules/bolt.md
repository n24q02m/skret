...
## 2026-09-09 - Zero-Allocation Path Parsing
**Learning:** Using `strings.Split` to parse path components unconditionally allocates a slice and performs work that may not be necessary if the string is only being validated, leading to memory overhead in hot paths.
**Action:** Replace `strings.Split(s, "/")` in validation functions with manual loops using `strings.IndexByte(s, '/')`. This reduces allocations to zero while remaining readable, especially for standard manifest and file path validation.
