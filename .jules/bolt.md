...
## 2026-08-30 - Optimize trailing path segment extraction
**Learning:** In string parsing routines that extract trailing segments, using `strings.Split(s, delim)` followed by `strings.Join` unconditionally allocates a slice and constructs a new string.
**Action:** Use a manual reverse-parsing loop with `strings.LastIndexByte` to slice directly from the original string. Use `strings.TrimLeft` safely to clean delimiters.
