...
## 2026-08-31 - Zero-Allocation Path Resolution
**Learning:** Using `strings.Split` followed by `strings.Join` for extracting path segments unconditionally allocates slices and strings. A manual reverse-parsing loop using `strings.LastIndexByte` directly on the string avoids these allocations and is significantly faster.
**Action:** Replace `strings.Split` and `strings.Join` with a manual reverse-parsing loop, and safely handle multiple or leading slashes by using `strings.TrimLeft(slice, "/")` instead of manual index bounds checking.
