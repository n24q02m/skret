...
## 2026-08-26 - Zero-allocation string parsing in path extraction
**Learning:** Using `strings.Split` and `strings.Join` unconditionally allocates slices and strings. For string parsing routines extracting trailing segments (e.g. Windows path extraction), we can use a manual reverse-parsing loop with `strings.LastIndexByte` to slice directly from the original string, achieving a zero-allocation fast path while drastically reducing overhead.
**Action:** Replace `strings.Split` and `strings.Join` with a manual reverse-parsing loop for string segment extraction on hot paths. Explicitly check for `-1` index and use `strings.TrimLeft(slice, "/")` on the slice to safely replicate delimiter-stripping without manual index arithmetic bounds checking.
