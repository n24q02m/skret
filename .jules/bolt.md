...
## 2026-05-19 - Optimize slog.Record Attribute Addition
**Learning:** Adding attributes to an `slog.Record` one-by-one in a loop using `AddAttrs(a)` introduces significant overhead due to repeated function calls and internal slice manipulations.
**Action:** When transferring or adding multiple attributes to a record (like in a logging middleware or `slog.Handler`), pre-allocate the required slice capacity using `make([]slog.Attr, 0, r.NumAttrs())` and use a single variadic call `newRecord.AddAttrs(attrs...)`.
