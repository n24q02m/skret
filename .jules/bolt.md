...
## 2026-08-16 - Optimize Path Resolution Allocations
**Learning:** In string parsing routines that extract trailing segments (e.g., path resolution), avoid using `strings.Split(s, delim)` followed by `strings.Join`. This unconditionally allocates a slice and constructs a new string.
**Action:** Use a manual reverse-parsing loop to slice directly from the original string, achieving zero/minimal allocations and significantly faster execution on hot paths.
