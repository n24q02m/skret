...
## 2026-08-18 - Zero-allocation path resolution
**Learning:** In string parsing routines that extract trailing segments, using `strings.Split(s, delim)` followed by `strings.Join` unconditionally allocates a string slice and constructs a new string. Replacing this pattern with a manual reverse-parsing loop that uses `strings.LastIndexByte` to slice directly from the original string achieves zero allocations and significantly faster execution (e.g., speeding up by ~7x in path resolution logic).
**Action:** Use manual reverse-parsing with `strings.LastIndexByte` instead of `strings.Split` + `strings.Join` when extracting or validating trailing path segments on critical paths.
