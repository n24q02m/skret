...
If `go.mod` specifies a Go version higher than what is available in the environment (e.g., Go 1.26 while the environment or linter only supports up to Go 1.24), it can cause CI failures, especially with `golangci-lint`. Downgrading the version in `go.mod` and `.golangci.yaml` to a compatible version (e.g., 1.24) can resolve these issues if the code doesn't rely on newer features.
