...
## 2026-08-14 - Avoid strings.Split in extract trailing path segments
**Learning:** In string parsing routines that extract trailing segments, using strings.Split followed by strings.Join unconditionally allocates a slice and constructs new strings, leading to high memory overhead.
**Action:** Replace strings.Split with manual reverse-parsing loops using direct backward index scanning to slice directly from the original string, achieving lower allocations and significantly faster execution.
