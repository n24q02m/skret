...
## 2026-05-08 - Optimize GetBatch Slice Allocation\n**Learning:** Dynamic slice appending inside loops forces the GC to reallocate arrays if the final size is not specified up front.\n**Action:** Always pre-allocate slice capacity using `make([]T, 0, len)` when the maximum size of a returned list is known, to reduce dynamic allocations inside performance-critical paths like bulk data fetching.
