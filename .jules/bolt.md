...
## 2026-08-24 - Optimize string parsing for trailing path segments
**Learning:** Using strings.Split followed by strings.Join to parse trailing path segments unconditionally allocates a slice and string.
**Action:** When extracting trailing segments, avoid strings.Split. Instead, use a manual reverse-parsing loop with strings.LastIndexByte to slice directly from the original string, achieving lower allocations and significantly faster execution. Use strings.TrimLeft on the resulting slice instead of manual index checks to handle delimiters safely.
