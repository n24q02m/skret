...
## 2026-08-19 - Avoid allocating strings.Split in shell-mangling path resolution
**Learning:** Shell-mangling recovery paths, like the one in `ResolvePath`, that scan backward for segments matching a pattern unconditionally allocated strings using `strings.Split` and `strings.ReplaceAll` when those paths can just be scanned dynamically, without memory allocations using reverse traversal and indices.
**Action:** Replace `strings.Split` arrays and `strings.ReplaceAll` operations with a zero-allocation backward string traversal loop, using index offsets to traverse segments dynamically reducing overhead and memory allocations.
