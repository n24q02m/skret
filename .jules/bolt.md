...
## 2024-05-18 - Avoid slice allocation with strings.Cut
**Learning:** `strings.Split` allocates a new slice on the heap. When splitting strings where the resulting slice isn't needed, such as when parsing and validating segments sequentially, replacing unconditional `strings.Split` with `strings.Cut` in a loop eliminates slice allocations and improves performance without significantly sacrificing code readability.
**Action:** Prefer `strings.Cut` over `strings.Split` when iterating over segments in a loop, especially on hot paths like validation.
