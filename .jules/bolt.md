...
## 2026-08-29 - Zero-Allocation Reverse Path Extraction
**Learning:** Using `strings.Split` and `strings.Join` for path extraction from the end of strings forces unconditional allocations for a slice and new string. Manual reverse-parsing loops using `strings.LastIndexByte` allow slicing directly from the original string, achieving zero slice allocations. However, when doing this, it's crucial to check `slashIdx == -1` to evaluate the remaining prefix and avoid out-of-bounds slice errors, and to use `strings.TrimLeft` instead of manual index offset when stripping delimiters for the final slice.
**Action:** When extracting trailing segments from path strings in Go, replace `strings.Split` / `strings.Join` with a manual reverse-parsing loop via `strings.LastIndexByte`. Ensure the `-1` condition is handled safely.
