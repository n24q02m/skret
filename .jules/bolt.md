...
## 2026-08-20 - Zero-Allocation Reverse String Parsing
**Learning:** In string parsing routines that extract trailing segments (e.g., path resolution), avoid using `strings.Split(s, delim)` followed by `strings.Join`. This unconditionally allocates a slice and constructs a new string. When refactoring to use zero-allocation manual reverse-loops (e.g., via `strings.LastIndexByte`), explicitly handle the edge case where the delimiter is not found (which returns `-1`). Returning or slicing with an unchecked `-1` index (e.g., `norm[idx:]`) will trigger an out-of-bounds runtime panic.
**Action:** Use a manual reverse-parsing loop with `strings.LastIndexByte` to slice directly from the original string, checking for `-1`, achieving zero allocations and significantly faster execution.
