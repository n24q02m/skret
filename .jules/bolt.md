...
## 2026-07-24 - Rejected: Micro-optimizing Non-hot Paths
**Learning:** Bypassing `strings.Builder` allocation for unescaped quoted dotenv values in `dotenv.Encode` provides negligible, unmeasured benefit on a non-hot path at the cost of readability. It was rejected as churn per maintainability policy because it violated the rule against micro-optimizations without a benchmark showing a measurable or hot-path win.
**Action:** Do not micro-optimize string allocations on non-hot paths (like dotenv encoding). Always measure the benefit and confirm the function is on a hot path before sacrificing idiomatic readable code for a fast-path.
