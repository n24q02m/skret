...
## $(date +%Y-%m-%d) - Zero-allocation string splitting loops
**Learning:** Functions like `strings.Split` provide a convenient API, but incur measurable memory allocation overhead because they return a slice. Replacing them with a manual loop using `strings.IndexByte` and slicing the original string avoids the heap allocation of the slice, providing a significant performance improvement (zero allocations) on hot paths while maintaining readability.
**Action:** Replace `strings.Split` in hot paths (like `cli.sync` target parsing, `auth.keyring` iteration) with manual `strings.IndexByte` loops to achieve zero allocations.
