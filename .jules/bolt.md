...
## 2026-08-17 - Avoiding string slice allocations in path parsing
**Learning:** In string parsing routines that extract trailing segments (e.g., path resolution), using `strings.Split(s, delim)` followed by `strings.Join` unconditionally allocates a slice and constructs a new string.
**Action:** Use a manual reverse-parsing loop to slice directly from the original string, achieving zero allocations and significantly faster execution.
