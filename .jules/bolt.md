...
## 2026-09-03 - Optimize ResolvePath with Zero-Alloc Reverse Parse
**Learning:** Path parsing logic that checks trailing string segments often uses `strings.Split` and `strings.Join`, allocating a slice and copying strings even when returning the unmodified string. Using a manual reverse loop with indexing achieves zero allocations for cold paths.
**Action:** When extracting trailing segments of a string separated by slashes/delimiters, use a manual reverse loop over the string length to check segment viability instead of splitting the entire string upfront.
