...
## 2026-09-10 - Zero-Allocation Path Validation in Go
**Learning:** Using `strings.Split(value, "/")` for path validation unconditionally allocates a slice and performs multiple string allocations, which can be a bottleneck in tight loops or frequent validations. A manual forward-parsing loop using `strings.IndexByte` eliminates these allocations entirely.
**Action:** Replace `strings.Split` with manual index tracking loops for simple validation logic where the resulting slice isn't actually needed.
