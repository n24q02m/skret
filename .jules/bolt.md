...
## 2026-09-01 - Zero-allocation trailing path segment extraction
**Learning:** Using `strings.Split` followed by `strings.Join` to parse and extract trailing path segments unconditionally allocates slices and new strings.
**Action:** When extracting known trailing path patterns, use a manual reverse-parsing loop with `strings.LastIndexByte` slicing directly from the original string. Ensure the `-1` not-found edge case is handled safely to avoid panics.
