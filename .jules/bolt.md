...
## 2024-05-18 - Avoid allocations on trailing path extraction
**Learning:** `strings.Split` followed by `strings.Join` for trailing path segment extraction allocates heavily and performs poorly compared to zero-allocation reverse loop approaches.
**Action:** Replace `strings.Split` and `strings.Join` combinations with manual reverse-parsing using `strings.LastIndexByte` where appropriate. Remember to handle `-1` appropriately to prevent index out of bounds, and check early exits to prevent scanning. When stripping delimiters from strings, `strings.TrimLeft(norm[lastIdx:], "/")` is a safe way to replicate `strings.Join` without allocating bounds check.
