...
## 2024-05-18 - Hoist strings.NewReplacer and add fast-path
**Learning:** `strings.NewReplacer` allocates memory and builds internal tables on initialization. Creating it inside a frequently called function like `sanitizeID` causes unnecessary CPU overhead and GC pressure.
**Action:** Always hoist `strings.NewReplacer` to a package-level variable so it's initialized only once. Additionally, add a fast-path check like `strings.ContainsAny` to skip replacement logic entirely if the target characters are not present in the string.
