...
## 2026-09-06 - Optimize string parsing in path resolution
**Learning:** Using `strings.Split` followed by `strings.Join` for extracting trailing path segments unconditionally allocates slices and strings, increasing GC pressure.
**Action:** Replace `strings.Split` with manual zero-allocation reverse-parsing using `strings.LastIndexByte`. When replacing `strings.Join`, use `strings.TrimLeft(slice, "/")` on the resulting slice to mimic delimiter stripping safely while preserving zero allocations.
