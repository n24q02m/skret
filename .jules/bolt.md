...
## 2026-08-15 - Zero-allocation reverse parsing for paths
**Learning:** In string parsing routines that extract trailing segments, using `strings.Split` followed by `strings.Join` unconditionally allocates a slice and a new string. A manual reverse-parsing loop slicing directly from the original string avoids these allocations and runs significantly faster.
**Action:** Avoid `strings.Split` + `strings.Join` for trailing path resolution. Use reverse parsing loops slicing original strings where possible.
