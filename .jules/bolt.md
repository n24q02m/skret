...
## 2026-10-28 - Hoist strings.NewReplacer
**Learning:** `strings.NewReplacer` allocates memory and builds internal replacement tables upon initialization. Initializing it inside frequently called functions creates redundant GC allocations and CPU overhead.
**Action:** Always hoist `strings.NewReplacer` declarations to package-level variables so they are initialized only once.
