...
## 2026-08-23 - Zero-Allocation Path Parsing
**Learning:** In string parsing routines that extract trailing segments (e.g., path resolution in `ResolvePath`), using `strings.Split(s, delim)` followed by `strings.Join` unconditionally allocates a slice and constructs a new string.
**Action:** Use a manual reverse-parsing loop with `strings.LastIndexByte` to slice directly from the original string, achieving zero allocations and significantly faster execution. Handle the edge case where the delimiter is not found to avoid out-of-bounds runtime panics.
