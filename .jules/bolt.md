...
## 2026-09-05 - Zero-allocation reverse path parsing
**Learning:** Using `strings.Split` and `strings.Join` for extracting valid trailing path segments creates unnecessary slices and allocates strings for every step.
**Action:** Use a manual reverse loop with `strings.LastIndexByte` to slice directly from the original string. Use `strings.TrimLeft` at the end to emulate `strings.Join`'s cleanup behavior. Always remember to check for `-1` to avoid panics.
