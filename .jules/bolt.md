...
## 2026-07-13 - Avoid package-level mutable slices for minor optimizations
**Learning:** Hoisting a small slice literal (like `[]byte{'\n'}`) to a package-level variable to avoid allocations in a loop introduces a mutable global state (`var nl = []byte{'\n'}`) which is a potential footgun. Given that modern Go compilers optimize non-escaping small slice literals in standard library calls, the allocation savings are virtually zero. This is a self-refuting optimization unless backed by a benchmark proving significant gains.
**Action:** Do not hoist small byte slices to package-level variables just to avoid loop allocations unless you have a benchmark proving a substantial performance improvement, and prefer immutable alternatives (like constants or strings) if a global is truly necessary.
