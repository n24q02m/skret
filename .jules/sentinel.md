...
## 2024-06-29 - [FIX] Corruption bug with dollar sign expansion
**Vulnerability:** Environment variable values containing `$` (e.g., bcrypt hashes, passwords) were at risk of being corrupted if the injection logic performed shell-style expansion.
**Learning:** The `internal/exec` package correctly avoids `os.ExpandEnv` to ensure byte-exact injection, which is critical for secret management.
**Prevention:** Use regression tests that explicitly check for verbatim preservation of `$` in various positions (leading, trailing, standalone) and within braced sequences.
