...
## 2026-08-17 - [Scrollable Region Accessibility]
**Learning:** Adding `tabindex="0"`, `role="region"`, and a dynamic `aria-label` to horizontally scrollable tables ensures that keyboard-only users can focus and scroll the container, and screen reader users are informed they've entered a scrollable region with clear context.
**Action:** Always include `tabindex="0"`, `role="region"`, and an `aria-label` on containers with `overflow-x: auto` or similar styles to ensure accessibility for scrollable areas.
