...
## 2026-09-18 - [Contextual ARIA Labels for Repeated Elements]
**Learning:** When a UI contains multiple regions or elements of the same type (e.g., multiple scrollable tables on a dashboard), each element's `aria-label` must incorporate contextual data (like its corresponding namespace or environment) to uniquely identify it. Using identical, generic labels across multiple elements degrades the experience for screen reader users as they cannot distinguish between them.
**Action:** Always interpolate contextual identifiers into `aria-label` attributes for repeated structural elements.
