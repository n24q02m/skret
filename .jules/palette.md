...
## 2026-08-31 - Add keyboard accessibility to scrollable secrets tables
**Learning:** Horizontally scrollable containers (`overflow-x: auto`) require explicit `tabindex="0"`, `role="region"`, and an `aria-label` to ensure keyboard accessibility and proper screen reader context for overflowing content. Additionally, native `:focus-visible` outlines should be applied to these containers to match existing interactive elements.
**Action:** When creating custom scrollable wrappers, always explicitly define keyboard focusability, semantic region roles, descriptive ARIA labels, and corresponding focus CSS rules to support both visual and non-visual navigation.
