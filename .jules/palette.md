...
## 2026-09-20 - Contextual ARIA Labels
**Learning:** When a UI contains multiple scrollable regions of the same type, identical `aria-label` attributes degrade the experience for screen reader users by failing to uniquely identify each element.
**Action:** Always incorporate contextual data (such as namespace and environment names) into the `aria-label` for repeating UI components.
