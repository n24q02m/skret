...
## 2026-09-17 - [Contextual ARIA Labels for Scrollable Regions]
**Learning:** When a UI contains multiple scrollable regions of the same type (like multiple tables), giving them identical `aria-label` attributes (e.g., "Secrets table") degrades the experience for screen reader users by failing to distinguish between them.
**Action:** Always include contextual data (such as the namespace or environment) in the `aria-label` of repeated scrollable regions to uniquely identify them and improve navigation clarity.
