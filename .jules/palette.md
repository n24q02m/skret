...
## 2026-09-06 - [Accessible Scrollable Regions]
**Learning:** Horizontally scrollable containers (e.g., tables with overflow) are inaccessible to keyboard users unless explicitly made focusable. Furthermore, using a generic `tabindex="0"` without semantic roles can confuse screen readers.
**Action:** Always add `tabindex="0"`, `role="region"`, and a descriptive `aria-label` to horizontally scrollable containers to ensure full keyboard and screen reader accessibility, and explicitly define `:focus-visible` CSS for visual feedback.
