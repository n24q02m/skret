...
## 2026-08-11 - Horizontally Scrollable Containers Accessibility
**Learning:** Horizontally scrollable containers (e.g., elements with `overflow-x: auto`) must include `tabindex="0"`, `role="region"`, and a descriptive `aria-label` to ensure keyboard accessibility and proper screen reader context for overflowing content. Otherwise, users navigating by keyboard cannot focus the container to scroll it, making the overflowing content completely inaccessible.
**Action:** When creating raw HTML elements or UI components with scrollable regions (like a table wrapper), ensure `tabindex="0"`, `role="region"`, and an `aria-label` are added to the container.
