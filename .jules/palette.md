...
## 2026-08-08 - Make overflow regions keyboard focusable
**Learning:** Elements styled with `overflow-x: auto` or `overflow: auto` that contain scrollable content must be given a `tabindex="0"` to allow keyboard-only users to scroll the region if the content overflows the viewport. In addition, providing a `role="region"` and a descriptive `aria-label` ensures screen readers correctly contextualize the focusable element.
**Action:** When creating custom horizontally-scrollable containers like data tables (`.tablewrap`), always ensure keyboard accessibility by making the region focusable, styling its `:focus-visible` state to match standard inputs, and assigning proper ARIA attributes to describe the region.
