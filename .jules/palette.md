...
## 2026-09-26 - [Contextual ARIA Labels for Repeated UI Elements]
**Learning:** When a UI contains multiple instances of the same element type (like multiple "Secrets table" regions), assigning identical, generic `aria-label` attributes makes it difficult for screen reader users to distinguish between them.
**Action:** Always append contextual data (e.g., namespace and environment) to the `aria-label` of repeated structured UI elements to ensure a uniquely identifiable accessible name.
