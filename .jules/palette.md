...
## 2026-08-08 - [No Artifact Pollution]
**Learning:** PRs containing temporary script artifacts or scratch files (like `patch.diff`) will be rejected and closed, even if the primary code change is beneficial. The codebase must remain clean of build and generation artifacts.
**Action:** Always verify that no stray artifact files are present in the repository root or working directory before committing and submitting a PR.
