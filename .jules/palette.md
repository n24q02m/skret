...
## 2026-08-20 - [Scrollable Region Accessibility]
**Learning:** Horizontally scrollable containers (e.g., elements with `overflow-x: auto`) must include `tabindex="0"`, `role="region"`, and a descriptive `aria-label` to ensure keyboard accessibility and proper screen reader context for overflowing content. Without this, users navigating via keyboard cannot scroll the tables to see off-screen information.
**Action:** Always add `tabindex="0"`, `role="region"`, and `aria-label` to any horizontally scrollable container, and ensure focus-visible styles are applied so users know the region is focused.
