...
## 2026-07-12 - [Appending to Journal Files]
**Learning:** Overwriting a journal file with a single new entry causes catastrophic data loss of all previous learnings. When adding entries to files like `.jules/palette.md`, it is critical to use append operations (`>>`) instead of overwrite operations (`>`), or to use `replace_with_git_merge_diff` to add the new entry to the end of the file.
**Action:** Never overwrite a journal file. Always append new learnings to the bottom of the existing `.jules/*.md` files to preserve the accumulated knowledge history.
