...
## 2026-08-10 - Keyboard Accessibility for Scrollable Tables
**Learning:** Scrollable regions (overflow-x: auto) without focusable elements cannot be scrolled by keyboard-only users. This is a common accessibility issue for data tables.
**Action:** Always add `tabindex="0"`, `role="region"`, and an `aria-label` to horizontally-scrollable container elements to ensure keyboard accessibility and proper screen reader context.
