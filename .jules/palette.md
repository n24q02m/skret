...
## 2026-09-16 - [Contextual ARIA Labels for Repeated UI Elements]
**Learning:** When a UI contains multiple regions of the same type (e.g., multiple scrollable tables for different namespaces), using an identical, generic `aria-label` (like "Secrets table") across all of them degrades the experience for screen reader users, as they cannot distinguish between the regions.
**Action:** Always incorporate contextual data (such as the namespace or environment name) into the `aria-label` of repeated elements to ensure they are uniquely identifiable to assistive technologies.
