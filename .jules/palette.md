...
## 2026-09-23 - Contextual ARIA labels for scrollable tables
**Learning:** Using identical generic `aria-label`s (like "Secrets table") across multiple scrollable regions degrades screen reader experience as users lose context of which section they are navigating.
**Action:** Always incorporate contextual data (like namespaces or environment names) into `aria-label` attributes when a UI contains multiple similar regions.
