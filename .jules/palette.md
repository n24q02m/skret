...
## 2026-09-09 - [Contextual ARIA Labels for Repeated UI Elements]
**Learning:** When a UI contains multiple regions or elements of the same type (e.g., multiple scrollable tables for different namespaces), each element's `aria-label` must incorporate contextual data (like its corresponding namespace or environment) to uniquely identify it. Using identical, generic labels across multiple elements degrades the experience for screen reader users.
**Action:** Always ensure that interactive or structural UI elements repeated in a list or dashboard have unique, contextual `aria-label` attributes that differentiate them from their siblings.
