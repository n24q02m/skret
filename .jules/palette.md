...
## 2026-08-26 - [Scrollable Regions Accessibility]
**Learning:** Horizontally scrollable containers (e.g., elements with `overflow-x: auto`) must include `tabindex="0"`, `role="region"`, and a descriptive `aria-label` to ensure keyboard accessibility and proper screen reader context for overflowing content. Furthermore, an explicit `:focus-visible` style must be applied.
**Action:** Always ensure scrollable data tables and similar regions include these attributes and visual focus states.
