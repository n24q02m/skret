...
## 2026-09-03 - [Scrollable Region Accessibility]
**Learning:** Horizontally scrollable containers (like `overflow-x: auto` tables) are unreachable by keyboard users unless explicitly made focusable. Adding `tabindex="0"` along with `role="region"` and an `aria-label` ensures screen readers announce the overflowing content properly and keyboard users can scroll it natively.
**Action:** Always include `tabindex="0"`, `role="region"`, and an appropriate `aria-label` on horizontally scrollable containers to ensure full keyboard and screen reader accessibility.
