...
describe("renderDashboard", () => {
  ...
  it("provides contextual aria-labels for each table wrapper", () => {
    const html = renderDashboard([m], FIXED_NOW);
    expect(html).toContain('aria-label="/klprism/prod prod secrets"');
  });
...
describe("renderLogin a11y", () => {
  ...
  it("exposes the error state in the document title", () => {
    expect(renderLogin()).toContain("<title>skret vault</title>");
    expect(renderLogin("wrong password")).toContain("<title>Error - skret vault</title>");
  });
});
