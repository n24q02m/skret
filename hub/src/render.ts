...
function renderNamespace(m: Manifest, now: number): string {
  ...
  return (
    ...
    `<div class="tablewrap" tabindex="0" role="region" aria-label="Secrets for ${esc(m.namespace)} ${esc(m.env)}"><table><thead><tr><th scope="col">Key</th><th scope="col">Fingerprint</th><th scope="col">Targets</th></tr></thead>` +
