...
const STYLE = `
  ...
  .tablewrap:focus-visible,a:focus-visible,input:focus-visible,button:focus-visible{outline:2px solid #1a1a1a;outline-offset:2px;border-radius:2px}
  .err{color:#8a1a1a}
...
function renderNamespace(m: Manifest, now: number): string {
  ...
  return (
    ...
    `<div class="tablewrap" tabindex="0" role="region" aria-label="Secrets table for ${esc(m.namespace)} ${esc(m.env)}"><table><thead><tr><th scope="col">Key</th><th scope="col">Fingerprint</th><th scope="col">Targets</th></tr></thead>` +
