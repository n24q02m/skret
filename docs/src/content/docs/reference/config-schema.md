...
| `no_overwrite` | bool | No | All | Only write keys absent at this target; existing keys are never overwritten. On the ordinary no-overwrite path, deleting a target key lets the next sync repopulate it. `skret sync --rotate` is the separate explicit overwrite intent: it overrides this setting, bypasses `--skip-unchanged`, records durable per-key outcomes, and cannot be combined with `--no-overwrite`. |
