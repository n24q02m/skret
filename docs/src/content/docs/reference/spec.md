...
**Version 1.0.0** — normative. The key words MUST, MUST NOT, SHOULD, and MAY are to be interpreted as described in RFC 2119.
...
When an environment sets `encrypted: true`, the local provider stores the secrets file as a keystore envelope. Readers auto-detect an envelope on disk regardless of the flag, so plaintext keeps working until `skret keys init --encrypt-existing` migrates it.

Envelope shape (YAML):

```yaml
version: "1"
format: skret-encrypted-v1     # marker; Detect() keys on this field
kdf:
  algorithm: argon2id
  salt: <base64>               # fresh per Seal
  time: 3
  memory_kib: 65536
  parallelism: 4
secrets:
  DATABASE_URL: "v1:<base64 nonce>:<base64 ciphertext+tag>"
meta:                          # optional, plaintext, never value-derived
  DATABASE_URL: "2026-10-01T00:00:00Z"   # per-key expiry (RFC3339)
```

- AEAD: ChaCha20-Poly1305, key derived with argon2id (parameters above; stored per-envelope so they can evolve). Each value's secret key name is bound as AEAD additional data — values cannot be transplanted between names.
- KDF parameters are stored in the envelope header and contain nothing value-derived; `meta` carries non-secret per-key metadata (currently expiry timestamps) as plaintext.
- Writes are atomic (temp file + rename) with `0600` permissions.
- Wrong key material fails with exit 4 (`ExitAuthError`) and a remediation hint; it is never a silent empty read.

Key-material precedence (first wins):

1. `SKRET_AGE_KEY`
2. `SKRET_LOCAL_KEY`
3. OS keyring entry (service `skret`, user `local-enc-key`) — where `skret keys init` stores generated material
